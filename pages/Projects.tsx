...
export default function Projects() {
  ...
  return (
    <>
      ...
      <section className="py-16 bg-gray-50 dark:bg-dark-bg min-h-screen">
                ...
                <button
                  ...
                  className={`px-6 py-2 rounded-full text-sm font-medium transition-all ${
                    filter === cat
                      ...
                      : 'bg-white dark:bg-dark-surface text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-700'
                  }`}
                >
                  {cat}
                </button>
            ...
            <AnimatePresence>
              {filteredProjects.map((project) => (
                <motion.div
                  ...
                  className="bg-white dark:bg-dark-surface rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all border border-gray-100 dark:border-dark-border group"
                >
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex flex-wrap gap-2 pt-4 border-t border-gray-100 dark:border-dark-border">
                       {project.techStack.slice(0, 3).map(t => (
                         <span key={t} className="text-xs text-gray-500 dark:text-gray-300 bg-gray-50 dark:bg-dark-card px-2 py-1 rounded">
                           {t}
                         </span>
                       ))}
                       {project.techStack.length > 3 && (
                         <span className="text-xs text-gray-500 dark:text-gray-300 px-1 py-1">+</span>
                       )}
                    </div>
                  </div>
                </motion.div>
